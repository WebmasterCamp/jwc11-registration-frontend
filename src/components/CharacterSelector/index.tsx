...
export default props => {
	...
	const move = (direction: 'left' | 'right') => {
		const majors = [Character.content, Character.design, Character.marketing, Character.programming];
		if (direction === 'left') {
			setSelector(majors[(majors.findIndex(major => major === selector) + 1) % majors.length]);
		} else {
			setSelector(
				majors[
	...
	const MobileContent = (
		<Row>
			<div onClick={() => move('left')} style={{ position: 'absolute', top: '50%', left: '0' }}>
				<FontAwesomeIcon icon="angle-left" style={{ color: '#ffbc5f', fontSize: '3em', cursor: 'pointer' }} />
			</div>
			<Card>
				<CharacterCard active={true} src={`/images/${selector}.png`} />
				<Description>Web {selector[0].toUpperCase() + selector.substr(1)}</Description>
			</Card>
			<div onClick={() => move('right')} style={{ position: 'absolute', top: '50%', right: '0' }}>
				<FontAwesomeIcon icon="angle-right" style={{ color: '#ffbc5f', fontSize: '3em', cursor: 'pointer' }} />
			</div>
		</Row>
	);
	return (
